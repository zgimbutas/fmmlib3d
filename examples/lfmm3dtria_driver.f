...
c       
      do i=1,ntri
         iff = 1
         ch = 1.0d0
         call lpotfld3d(iff,source(1,1),ch,centroids(1,i),potd,fldd)
c       
...
cccC$OMP$NUM_THREADS(1) 
      do j=1,m
         if (ifslp .ne. 0) then
            call direct3dtrilaps2(j,ntri,centroids,charge,
     $          triangles,ifpot,ptemp,iffld,ftemp)
         ...
         if (ifdlp .ne. 0) then
            call direct3dtrilapd2(j,ntri,centroids,dipstr,
     $        triangles,trinorm,ifpot,ptemp,iffld,ftemp)
            if( ifpot .eq. 1) pot2(j) = pot2(j) + ptemp
            if( iffld .eq. 1) then
               fld2(1,j) = fld2(1,j) + ftemp(1)
               fld2(2,j) = fld2(2,j) + ftemp(2)
...
cccC$OMP$NUM_THREADS(4) 
      do j=1,m
         if (ifslp .ne. 0) then
            call direct3dtrilaps2(j,ntri,target,charge,
     $           triangles,ifpottarg,ptemp,iffldtarg,ftemp)
         ...
         if (ifdlp .ne. 0) then
            call direct3dtrilapd2(j,ntri,target,dipstr,
     $         triangles,trinorm,ifpottarg,ptemp,iffldtarg,ftemp)
            if( ifpottarg .eq. 1) pot2(j) = pot2(j) + ptemp
            if( iffldtarg .eq. 1) then
              fld2(1,j) = fld2(1,j) + ftemp(1)
              fld2(2,j) = fld2(2,j) + ftemp(2)
